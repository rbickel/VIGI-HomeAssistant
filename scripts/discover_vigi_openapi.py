...
class VigiOpenApiDiscovery:
    ...
    def fetch_spec_candidates(self, token: str, paths: Iterable[str]) -> list[HttpResult]:
...
def render_endpoint_probe_markdown(
    ...
    lines = [
        ...
        "",
        "Generated by `scripts/discover_vigi_openapi.py` using the fixed endpoint list from the VIGI NVR Open API Document.",
        "",
        "## Read-Only Probes",
        "",
    ...
    lines.extend(
        [
            ...
            "## Alarm Finding",
            "",
            "The PDF exposes event push configuration and alarm-related event payload fields (`alarm_output`, `alarm_input`, event subtype `Alarm signal`, and device exception subtype `Alarm input`). It does not document arm/disarm/activate/deactivate control endpoints.",
            "",
        ]
    )
    return "\n".join(lines)
...
def render_endpoints_markdown(
    ...
    if alarm_rows:
        ...
        lines.append("No alarm candidates were found in the downloaded OpenAPI documents.")
...
def append_endpoint_table(lines: list[str], rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        lines.append(
            f"| {row['method']} | `{row['path']}` | {escape_cell(row['operation_id'])} | "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--port", type=int, default=int(os.environ.get("VIGI_PORT", "20443")))
...
def main() -> int:
    ...
    if args.probe_known_endpoints:
        added_devices = client.fetch_endpoint(token, "GET", "/openapi/added_devices", None)
        ...
        write_endpoint_probe_outputs(args.output_dir, endpoint_results, MUTATING_ENDPOINTS)
